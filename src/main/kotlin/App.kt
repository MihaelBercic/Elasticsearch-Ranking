...
class App(val client: ElasticsearchClient) {
    ...
    private val searchQueueElasticsearch = LinkedBlockingQueue<String>()
    private val searchQueueSQLite = LinkedBlockingQueue<String>()
    private val firstListModel = DefaultListModel<String>()
    private val secondListModel = DefaultListModel<String>()
                ...
                override fun keyReleased(e: KeyEvent?) {
                    ...
                    if (e?.keyCode == KeyEvent.VK_ENTER) {
                        ...
                        runBlocking {
                            ...
                            launch { searchSQLite(sqLabel, searchString) }
                        }
                    }
                }
    ...
    fun searchElastic(label: JLabel, word: String) {
        ...
        firstListModel.addAll(results.map { it.word })
    ...
    fun searchSQLite(label: JLabel, word: String) {
        val start = System.currentTimeMillis()
        val words = transaction {
            Words.select { Words.word like "$word%" }.limit(100).map { it[Words.word] }
        }
        secondListModel.clear()
        secondListModel.addAll(words)
        val end = System.currentTimeMillis()
        val total = end - start
        label.text = "${total}ms - (${words.size}) SQLite"
        println("SQ: ${total}ms \t ${words.size}")
    }
